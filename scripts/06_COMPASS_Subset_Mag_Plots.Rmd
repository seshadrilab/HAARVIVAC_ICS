...
```{r}
# gs_sub <- subset(gs, !(`SAMPLE ID` == "242C-2") &
#                    !(`SAMPLE ID` == "54H-2" & Stim == "S2") &
#                    !(`SAMPLE ID` == "161C-1" & Stim == "S2") &
#                    !(`SAMPLE ID` == "161C-2" & Stim %in% c("S2", "NCAP")))
...
plot_subset_vs_group <- function(bg_corr_boxplots, current_run,
  ...
  current_coreceptor <- sub("([A-Z][A-Z][0-9])_.*", "\\1", "CD4_S1")
current_stim <- sub("[A-Z][A-Z][0-9]_(.*)", "\\1", "CD4_S1")
