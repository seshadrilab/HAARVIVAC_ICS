...
group_labs <- list("Naive" = c("Naive PRE" = "Naive PRE", "Naive POST" = "Naive POST"),
                   "Conv" = c("Conv PRE" = "Conv PRE", "Conv POST" = "Conv POST"),
                   "PRE" = c("Naive PRE" = "Naive PRE", "Conv PRE" = "Conv PRE"))
```

```{r}
plot_subset_vs_group <- function(bg_corr_boxplots, current_run,
                                 current_subset, current_group,
...
current_stim <- sub("[A-Z][A-Z][0-9]_(.*)", "\\1", current_run)
  ...
  current_plot <- ggplot(current_data, aes(Group, BgCorr_Zeroed)) +
    theme_bw(base_size = 22) +
      ...
      scale_x_discrete(labels=group_labs[[current_group]], expand = c(0.3, 0.3)) +
      labs(y = sprintf("%% Responding %s T cells", current_coreceptor),
           title = current_stim,
           subtitle = current_subset_trunc) +
      scale_fill_manual(values = fill_colors[[current_group]]) +
    scale_color_manual(values = outline_colors[[current_group]])
...
cd4_s1_n_4 <- plot_subset_vs_group(cd4_naive_bg_corr_boxplots, "CD4_S1",
                     ...
                     fill_colors, outline_colors, group_labs,
                     current_group = "Naive",
