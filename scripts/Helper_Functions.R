...
make_dotplot_for_COMPASS_run <- function(cr, run_name, output_folder=NA, current_ylim=NULL, add_legend=FALSE, legend_position=c(0.16, 0),
                                         ...
                                         group_by_order=c("Naive PRE", "Naive POST", "Conv PRE", "Conv POST"), 
                                         group_by_colors=c("Naive PRE" = "#b7e3dc", "Naive POST" = "#49b7a6", "Conv PRE" = "#e9d5a6", "Conv POST" = "#d0a540"),
  ...
  tests <- lapply(compassSubsetsFiltered, function(boolSubset) {
    kruskal.test(as.formula(sprintf("`%s` ~ %s", boolSubset, group_by_colname)), data=dat_bgCorr_wide)
  })
  ...
  if(save_test_results) {
    ...
    if(!is.na(output_folder)) {
      test_results_file_path <- file.path(output_folder,
                                          sprintf("%s_%s_BooleanSubsets_BgCorrProps_KruskalWallis.tsv",
...
make_boxplot_for_COMPASS_run <- function(cr, subset_keep = NULL, run_name, output_folder=NA, current_ylim=NULL, add_legend=FALSE, legend_position=c(0.02, 1),
                                         ...
                                         cats_heatmap_left_padding = 3, main_title = NULL, onlyShowPBelowAlpha = TRUE) {
  # zeroed_BgCorr_stats If TRUE, the returned magnitude values and statistics are calculated using zeroed values
  # zeroed_BgCorr_plot If TRUE, the points on the plot are zeroed (regardless of zeroed_BgCorr_stats)
  library(ComplexHeatmap)
  library(grid)
...
# Boxplot of background-corrected subset percents out of the parent population
# All samples (naive and convalescent) at all time points (PRE and POST)
make_boxplot_for_COMPASS_run_all <- function(cr, run_name, output_folder=NA, current_ylim=NULL, add_legend=FALSE, legend_position=c(0.02, 1),
                                         save_test_results=TRUE, p_text_size=5, include_0_line=FALSE, zeroed_BgCorr_stats = FALSE, zeroed_BgCorr_plot = FALSE, 
                                         plot_width=7, plot_height=6, dichotomize_by_cytokine=NULL, dichotomize_by_cytokine_color=NULL, group_by_colname="Group",
                                         group_by_order=c("Hospitalized", "Non-hospitalized"),
                                         group_by_colors=c("Hospitalized" = "#757bbcb2", "Non-hospitalized" = "#b0d2c8bf"),
  ...
  tests1 <- lapply(compassSubsetsFiltered, function(boolSubset) {
    wilcox.test(as.formula(sprintf("`%s` ~ %s", boolSubset, group_by_colname)), data=dplyr::filter(dat_bgCorr_wide, Group %in% c("Naive PRE", "Naive POST")), paired=T)
  })
  tests2 <- lapply(compassSubsetsFiltered, function(boolSubset) {
    wilcox.test(as.formula(sprintf("`%s` ~ %s", boolSubset, group_by_colname)), data=dplyr::filter(dat_bgCorr_wide, Group %in% c("Conv PRE", "Conv POST")), paired = T)
  })
  tests3 <- lapply(compassSubsetsFiltered, function(boolSubset) {
    wilcox.test(as.formula(sprintf("`%s` ~ %s", boolSubset, group_by_colname)), data=dplyr::filter(dat_bgCorr_wide, Group %in% c("Naive PRE", "Conv PRE")))
  })
  tests4 <- lapply(compassSubsetsFiltered, function(boolSubset) {
    wilcox.test(as.formula(sprintf("`%s` ~ %s", boolSubset, group_by_colname)), data=dplyr::filter(dat_bgCorr_wide, Group %in% c("Naive POST", "Conv POST")))
  })
  pvals_df1 <- data.frame(BooleanSubset = compassSubsetsFiltered,
                          p = unlist(lapply(tests1, function(x) {x$p.value}))) %>% 
    mutate(p.adj = p.adjust(p, method = "bonferroni")) %>% # Strict
    mutate(p.adj.text = if_else(p.adj < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.adj, 3)))))
  pvals_df2 <- data.frame(BooleanSubset = compassSubsetsFiltered,
                          p = unlist(lapply(tests2, function(x) {x$p.value}))) %>% 
    mutate(p.adj = p.adjust(p, method = "bonferroni")) %>% # Strict
    mutate(p.adj.text = if_else(p.adj < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.adj, 3)))))
  pvals_df3 <- data.frame(BooleanSubset = compassSubsetsFiltered,
                          p = unlist(lapply(tests3, function(x) {x$p.value}))) %>% 
    mutate(p.adj = p.adjust(p, method = "bonferroni")) %>% # Strict
    mutate(p.adj.text = if_else(p.adj < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.adj, 3)))))
  pvals_df4 <- data.frame(BooleanSubset = compassSubsetsFiltered,
                          p = unlist(lapply(tests4, function(x) {x$p.value}))) %>% 
    mutate(p.adj = p.adjust(p, method = "bonferroni")) %>% # Strict
    mutate(p.adj.text = if_else(p.adj < 0.001, "p<.001", paste0("p=", sub("0.", ".", round(p.adj, 3)))))
  ...
  # Use the cats df row order to order the boolean subsets in dat_bgCorr_long and pvals_df
  dat_bgCorr_long$BooleanSubset <- factor(dat_bgCorr_long$BooleanSubset, levels = rownames(cats))
  pvals_df1$BooleanSubset <- factor(pvals_df1$BooleanSubset, levels = rownames(cats))
  pvals_df2$BooleanSubset <- factor(pvals_df2$BooleanSubset, levels = rownames(cats))
  pvals_df3$BooleanSubset <- factor(pvals_df3$BooleanSubset, levels = rownames(cats))
  pvals_df4$BooleanSubset <- factor(pvals_df4$BooleanSubset, levels = rownames(cats))
  ...
  if(save_test_results) {
    # Save some form of the test results to disk so it doesn't only exist in the plot as adjusted p-values
    pvals_df_for_file1 <- cats %>% rownames_to_column("BooleanSubset") %>% 
      dplyr::left_join(pvals_df1) %>% 
      dplyr::left_join(dat_bgCorr_medians %>%
                         pivot_wider(id_cols = "BooleanSubset",
                                     names_from = !!as.symbol(group_by_colname),
                                     values_from = BgCorr,
    ...
    pvals_df_for_file2 <- cats %>% rownames_to_column("BooleanSubset") %>% 
      dplyr::left_join(pvals_df2) %>% 
      dplyr::left_join(dat_bgCorr_medians %>%
                         pivot_wider(id_cols = "BooleanSubset",
                                     names_from = !!as.symbol(group_by_colname),
                                     values_from = BgCorr,
    ...
    pvals_df_for_file3 <- cats %>% rownames_to_column("BooleanSubset") %>% 
      dplyr::left_join(pvals_df3) %>% 
      dplyr::left_join(dat_bgCorr_medians %>%
                         pivot_wider(id_cols = "BooleanSubset",
                                     names_from = !!as.symbol(group_by_colname),
                                     values_from = BgCorr,
    ...
    pvals_df_for_file4 <- cats %>% rownames_to_column("BooleanSubset") %>% 
      dplyr::left_join(pvals_df4) %>% 
      dplyr::left_join(dat_bgCorr_medians %>%
                         pivot_wider(id_cols = "BooleanSubset",
                                     names_from = !!as.symbol(group_by_colname),
                                     values_from = BgCorr,
    ...
    if(!is.na(output_folder)) {
      test_results_file_path1 <- file.path(output_folder,
                                          sprintf("%s_%s_BooleanSubsets_BgCorrProps_MannWhitney_NaivePREPOST.tsv",
                                                  run_name, if(zeroed_BgCorr_stats) {"Zeroed"} else {"NotZeroed"}))
      write.table(pvals_df_for_file1,
                  file = test_results_file_path1,
                  sep = "\t", row.names = FALSE, quote = FALSE)
      
      test_results_file_path2 <- file.path(output_folder,
                                           sprintf("%s_%s_BooleanSubsets_BgCorrProps_MannWhitney_ConvPREPOST.tsv",
                                                   run_name, if(zeroed_BgCorr_stats) {"Zeroed"} else {"NotZeroed"}))
      write.table(pvals_df_for_file2,
                  file = test_results_file_path2,
                  sep = "\t", row.names = FALSE, quote = FALSE)
      
      test_results_file_path3 <- file.path(output_folder,
                                           sprintf("%s_%s_BooleanSubsets_BgCorrProps_MannWhitney_NaiveConvPRE.tsv",
                                                   run_name, if(zeroed_BgCorr_stats) {"Zeroed"} else {"NotZeroed"}))
      write.table(pvals_df_for_file3,
                  file = test_results_file_path3,
                  sep = "\t", row.names = FALSE, quote = FALSE)
      
      test_results_file_path4 <- file.path(output_folder,
                                           sprintf("%s_%s_BooleanSubsets_BgCorrProps_MannWhitney_NaiveConvPOST.tsv",
                                                   run_name, if(zeroed_BgCorr_stats) {"Zeroed"} else {"NotZeroed"}))
      write.table(pvals_df_for_file4,
                  file = test_results_file_path4,
                  sep = "\t", row.names = FALSE, quote = FALSE)
    }
  }
  ...
  if(showSignificanceBracket) {
    ...
    annotation_df1 <- pvals_df1 %>% 
      mutate(start = group_by_order[[1]],
             end = group_by_order[[2]],
             y_pos = get_y_pos(BooleanSubset))
    if(onlyShowPBelowAlpha) {
      annotation_df1 <- annotation_df1 %>% dplyr::filter(p.adj < p_alpha)
    }
    annotation_df2 <- pvals_df2 %>% 
      mutate(start = group_by_order[[3]],
             end = group_by_order[[4]],
             y_pos = get_y_pos(BooleanSubset))
    if(onlyShowPBelowAlpha) {
      annotation_df2 <- annotation_df2 %>% dplyr::filter(p.adj < p_alpha)
    }
    annotation_df3 <- pvals_df3 %>% 
      mutate(start = group_by_order[[1]],
             end = group_by_order[[3]],
             y_pos = get_y_pos(BooleanSubset))
    if(onlyShowPBelowAlpha) {
      annotation_df3 <- annotation_df3 %>% dplyr::filter(p.adj < p_alpha)
    }
    annotation_df4 <- pvals_df4 %>% 
      mutate(start = group_by_order[[2]],
             end = group_by_order[[4]],
             y_pos = get_y_pos(BooleanSubset))
    if(onlyShowPBelowAlpha) {
      annotation_df4 <- annotation_df4 %>% dplyr::filter(p.adj < p_alpha)
    }
    annotation_df <- bind_rows(annotation_df1, annotation_df2) %>%
      bind_rows(annotation_df3) %>%
      bind_rows(annotation_df4)
  ...
  to_return <- list("Plot" = combined_plot,
                    "BgCorrMagnitudes" = dat_bgCorr_wide)
  if(save_test_results) {
    to_return$Test_Results <- c(pvals_df_for_file1, pvals_df_for_file2, 
                                pvals_df_for_file3, pvals_df_for_file4)
  }
  to_return
}
...
# Plots of functionality scores and polyfunctionality scores
# Use Wilcoxon signed-rank test to compare across time within a patient and
# Wilcoxon rank-sum test to compare across patient groups within a timepoint.
# Show the medians in each group.
# Note: all p-values unadjusted

fs_pfs_plot <- function(FS_or_PFS = "FS", current_stim = "S1", cd4_or_cd8 = "CD4") {
  d <- fs_pfs_df %>% dplyr::filter(Stim == current_stim & parent == cd4_or_cd8)
  d_pre <- d %>% dplyr::filter(Timepoint == "PRE")
  d_post <- d %>% dplyr::filter(Timepoint == "POST")
  ...
  ggplot(d, aes(Group, !!as.name(FS_or_PFS))) +
    ...
    scale_x_discrete(expand = c(0.1, 0.1),
                     labels = c("Naive PRE" = "Naive\nPRE", "Naive POST" = "Naive\nPOST",
                                "Conv PRE" = "Conv\nPRE", "Conv POST" = "Conv\nPOST")) + 
...
split_fs_pfs_plot <- function(FS_or_PFS = "FS", current_stim = "S1",
  ...
  if(compare_intra){
    ...
    plot + geom_quasirandom(fill = "grey", color = "grey", width = 0.1)
  }
}