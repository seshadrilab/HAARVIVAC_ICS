...
cd4_naive_bg_corr_boxplots <- pmap(.l = list(CD4RunNames, 
                         ...
                         .f = function(n, p, y, p_text_size, left_cats_heatmap_padding, dichotomize_by_cytokine, main_title) {
                           ...
                           make_boxplot_for_COMPASS_run(crList[[n]], subset_keep = "H-", n, parentSubset = p, add_legend = T, paired = TRUE, 
                                                        ...
                                                        save_test_results=TRUE, output_folder=here::here("processed_data"))
                         })
...
```{r}
fs_plots <- pmap(list(rep(c("S1", "S2", "NCAP"), times = 2),
                           rep(c("CD4", "CD8"), each = 3)),
                           function(current_stim, cd4_or_cd8) {
                             fs_pfs_plot("FS", current_stim, cd4_or_cd8)
                           })
names(fs_plots) <- paste0(rep(c("CD4", "CD8"), each = 3), "_", rep(c("S1", "S2", "NCAP"), times = 2))
fs_plots
```
...
```{r}
pfs_plots <- pmap(list(rep(c("S1", "S2", "NCAP"), times = 2),
                           rep(c("CD4", "CD8"), each = 3)),
                           function(current_stim, cd4_or_cd8) {
                             fs_pfs_plot("PFS", current_stim, cd4_or_cd8)
                           })
names(pfs_plots) <- paste0(rep(c("CD4", "CD8"), each = 3), "_", rep(c("S1", "S2", "NCAP"), times = 2))
pfs_plots
```
