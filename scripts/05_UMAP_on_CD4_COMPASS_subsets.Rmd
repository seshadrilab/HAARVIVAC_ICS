...
print(
  ...
  # Second row
  (mem_legend | mem_plot_colored_by_gate | plot_spacer() |
...
Just HLADR, CD38, and PolyF plots alone.
...
Memory column was already added earlier, so subset data.
...
Set ggplot settings (also use these for the activation plots).
...
actColorScheme <- c("HLADR-/CD38-" = "#999999", "HLADR+/CD38+" = "#D55E00", "HLADR+/CD38-" = "#56B4E9", "HLADR-/CD38+" = "#009E73")
```
...
act_plot_colored_by_gate <- gg_themes +
  geom_point_rast(aes(colour=cd4_compass_subsets_w_umap[,"ActivationCategory"]),
               shape=20, alpha=0.8, size=pointSize) +
  ggtitle("Activation") +
...
```{r, fig.width=15, fig.height=5}
print(naive_polyf_plot | polyf_legend_vertical)
print(conv_polyf_plot | polyf_legend_vertical)
```

# Memory, Activation, and PolyF together

## Naive

```{r, fig.width=10, fig.height=2.5}
print(naive_act_plot_colored_by_gate | act_legend | naive_mem_plot_colored_by_gate | mem_legend | naive_polyf_plot | polyf_legend_vertical)
```

## Conv

```{r, fig.width=10, fig.height=2.5}
print(conv_act_plot_colored_by_gate | act_legend | conv_mem_plot_colored_by_gate | mem_legend | conv_polyf_plot | polyf_legend_vertical)
```